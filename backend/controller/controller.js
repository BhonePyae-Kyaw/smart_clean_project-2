...
const getOrderIdAtPaymentWaitingStage = async (req, res) => {
  ...
  try {
    const results = await client.query(
      queries.getOrderIdAtPaymentWaitingStage,
      [id]
    );
...
const addemail = async (req, res) => {
  ...
  try {
    ...
    if (checkResult.rows.length) {
      ...
      await client.query(queries.addemail, [
        email,
        name,
        phone,
        password,
...
const placeOrder = async (req, res) => {
  ...
  try {
    const result = await client.query(queries.placeOrder, [
      customer_id,
      pick_up_date,
      pick_up_time,
      status,
...
const updateTimeSchedule = async (req, res) => {
  ...
  try {
    const result = await client.query(queries.updateTimSchedule, [
      customer_id,
      pick_up_date,
      pick_up_time,
    ]);
...
const updatecusinfo = async (req, res) => {
  ...
  try {
    ...
    const customerExistsResult = await client.query(queries.getcusinfo, [
      customerId,
    ]);
    ...
    const updateQuery = `UPDATE customer SET ${setClause
      .map((clause, index) => `${clause} = $${index + 1}`)
      .join(", ")} WHERE customer_id = $${setClause.length + 1}`;
    ...
    res
      .status(200)
      .json({ message: "Customer information updated successfully" });
  } catch (error) {
...
const getcusorderhistory = async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error retrieving order history for customer:",
      error.message
    );
...
const createTimeSlot = async (req, res) => {
  ...
  try {
    const result = await client.query(queries.createTimeSlot, [
      pick_up_date,
      pick_up_time,
    ]);
...
const placeOrderItems = async (req, res) => {
  ...
  try {
    const result = await client.query(queries.placeOrderItems, [
      order_id,
      quantity,
      selling_price,
      item_id,
